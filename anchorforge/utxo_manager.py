...
This tool provides statistics, comparison with the blockchain, and repair
functionalities to ensure the local UTXO cache is accurate and to prevent
'Missing inputs' errors during transaction broadcasting.

--- Example Usage ---

Note: Requires either --address (preferred) or --file to target a wallet.
The --network flag is crucial when using --address.

--- 1. Analysis Commands (Read-Only) ---

# Get statistics for the mainnet wallet associated with a specific address
# (Derives filename 'utxo_store_main_...<addr_short>.json' automatically)
python utxo_manager.py --address 1Z3YVKKVQeE9jDgvdNgiLgTizdC7iqypa --network main stats

# Get statistics for a specific testnet file
python utxo_manager.py --file AF_py/utxo_store_test_mj2GSy6g.json --network test stats

# Compare the local mainnet file (derived from address) with the blockchain
# (This finds 'stale' UTXOs that cause 'Missing inputs' errors)
python utxo_manager.py --address 1Z3YVKKVQeE9jDgvdNgiLgTizdC7iqypa --network main compare

# Check the status of one specific UTXO (local vs. remote)
python utxo_manager.py --address 1Z3YVKKVQeE9jDgvdNgiLgTizdC7iqypa --network main check <txid>:<vout>

--- 2. Repair Commands (Write Actions) ---

# "Smart Repair": Compares local and remote, adds new UTXOs,
# and archives stale UTXOs to '..._invalid.json'. (Recommended)
python utxo_manager.py --address 1Z3YVKKVQeE9jDgvdNgiLgTizdC7iqypa --network main repair

# "Full Repair": Wipes the local file and re-downloads all
# UTXOs from the blockchain. (The "sledgehammer" method)
python utxo_manager.py --address 1Z3YVKKVQeE9jDgvdNgiLgTizdC7iqypa --network main full-repair

--- 3. Manual Override Commands (Write Actions) ---

# Manually remove a single 'stale' UTXO from the local file
python utxo_manager.py --address 1Z3YVKKVQeE9jDgvdNgiLgTizdC7iqypa --network main remove <txid>:<vout>

# Manually add a UTXO. The tool will first validate it on-chain
# before adding it to the local file.
# NOTE: 'add' still uses 'txid:vout:satoshis' format for input,
# but validates against the API's 'tx_hash' format.
python utxo_manager.py --address 1Z3YVKKVQeE9jDgvdNgiLgTizdC7iqypa --network main add <txid>:<vout>:<satoshis>
"""
...
def _resolve_filenames(args: argparse.Namespace) -> Dict[str, str]:
    """
    ...
    if args.file:
        ...
        try:
            ...
            logger.info(f"Resolved INVALID file: {invalid_file}")
        except Exception as e:
...
async def do_full_repair(args: argparse.Namespace, filenames: Dict[str, str]):
    ...
    (Version 2.2: Fixed to use correct API keys 'tx_hash'/'tx_pos'/'value')
    """
    ...
    skipped_count = 0
    for utxo in remote_utxos_list:
        # --- FIX (Version 2.2): Check for API keys ---
        if not all(k in utxo for k in (api_keys['txid_key'], api_keys['vout_key'], api_keys['sats_key'])):
            ...
            continue
        # --- END FIX ---
        formatted_utxos_for_store.append(_format_utxo_entry(utxo))
    ...
    logger.info("Full repair complete. Local file is now in sync.")
